...
def sample(path=None, dir_path=None, score_model=None, alpha_0=1.8,alpha_1=2, beta_min=0.1, beta_max=20,
           ...
           clamp=10, initial_clamp=3, final_clamp=1, device='cuda', datasets="CelebA", ch=128, ch_mult=[1, 2, 2,2,4],
           num_res_blocks=2, resolution=64, mode= 'approximation',h=1.5,
           clamp_mode="constant"):
    sde = VPSDE(alpha_0=alpha_0,alpha_1=alpha_1, beta_min=beta_min, beta_max=beta_max)
    ...
    name = str(name) + str(datasets) + str(
        time.strftime('%m%d_%H%M_', time.localtime(time.time()))) + '_' + 'alpha_1' + str(f'{alpha_1}') + 'beta' + str(
        f'{beta_min}') + '_' + str(
